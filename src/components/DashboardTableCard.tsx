...
import type { AssetClassTableRow, AssetTableRow } from '../domain/selectors/tableRows'
  ...
  return (
    <Card title="Positions">
      <div className="overflow-hidden rounded-lg border border-[rgb(var(--border))]">
        <div
          className={`grid items-center gap-4 border-b border-[rgb(var(--border))] bg-slate-50 px-4 py-2 text-xs font-semibold uppercase tracking-wide text-slate-500 ${
            mode === 'asset'
        ...
        >
          ...
          {mode === 'asset' ? <span className="text-right">Avg Price</span> : null}
        ...
        <div className="divide-y divide-[rgb(var(--border))]">
          ...
          {rows.map((row) => {
            ...
            return (
              <button
                key={row.key}
                className={`grid w-full items-center gap-4 px-4 py-3 text-left text-sm transition ${
                  isActive
